...
const String lexiKitContactText = 'Contact: geir.offenberg@gmail.com';
...
const Map<ToolType, ToolInstruction> lexiKitInstructions = {
  ToolType.wordFinder: ToolInstruction(
    title: 'Word Finder',
    body:
        'Enter a group of letters. LexiKit lists every dictionary word (minimum 2 letters) it can make from them, sorted so the most useful plays appear first. Separate your letters with spaces from any filters: add exact:N (that exact length only), min:N, max:N, starts:XY, ends:XY, or has:XY (must contain that text). Up to two wildcards (?, *, or _) count as blank tiles. This replaces the old Scrabble Finder and Unscrambler.',
  ),
